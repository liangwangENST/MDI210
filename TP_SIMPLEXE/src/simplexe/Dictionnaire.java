...
public class Dictionnaire extends Observable {
	...
	public int chercherPremierIndiceVariableEntrante() {
		 ...
		 for (int i = 1; i != nbVarHorsBase + 1; i++) {
	            if (D[0][i] > 0) {
	                return i;
	            }
	        }
	 ...
	 */
	 public void pivoter(int iS, int jE) {
	        ...
	        double factor = -D[iS][jE];
	        for (int j = 0; j != nbVarHorsBase + 1; j++) {
	            D[iS][j] /= factor;
	        }
	        double[] coeffs = new double[nbVarBase + 1];
	        for (int i = 0; i != nbVarBase + 1; i++) {
	            if (i == iS) {
	                coeffs[i] = -1 / factor;
	            } else {
	                coeffs[i] = D[i][jE] / D[iS][jE];
	                for (int j = 0; j != nbVarHorsBase + 1; j++) {
	                    D[i][j] = D[i][j] - coeffs[i] * D[iS][j];
	                }
	                coeffs[i] /= factor;
	            }
	        }
	        ...
	        int varL = tabVarBase[iS];
	        int varE = tabVarHorsBase[jE];
	        tabVarBase[iS] = varE;
	        tabVarHorsBase[jE] = varL;
	    }
